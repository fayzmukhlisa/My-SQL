...
--TASK1;
SELECT Category, COUNT(ProductID) AS TotalProduct FROM Products GROUP BY Category;
--TASK2;
SELECT AVG(Price) AS AvgPrice FROM Products WHERE Category = 'Electronics';
SELECT COUNT(*) AS TotalProducts, AVG(Price) AS AvgPrice FROM Products WHERE Category = 'Electronics';
...
--TASK3;
SELECT*FROM Customers WHERE City LIKE 'L%'; --or
SELECT FirstName, LastName, City FROM Customers WHERE City LIKE 'L%';
--TASK4;
SELECT*FROM Products WHERE ProductName LIKE '%er'; --or
SELECT ProductName FROM Products WHERE ProductName LIKE '%er';
--TASK5; 
SELECT*FROM Customers WHERE Country LIKE '%A'; --or
SELECT FirstName, LastName, Country FROM Customers WHERE Country LIKE '%A'; 
--TASK6;
SELECT MAX(Price) AS MaxPrice FROM Products; --or
SELECT ProductName, Price FROM Products WHERE Price = (SELECT MAX(Price) FROM Products);
--TASK7;
SELECT ProductName, StockQuantity, CASE WHEN StockQuantity < 30 THEN 'LowStock' ELSE 'Sufficient' END AS StockStatus FROM Products;
--TASK8;
SELECT Country, COUNT(CustomerID) AS TotalNumberCustomer FROM Customers GROUP BY Country; --or 
SELECT Country, COUNT(*) AS TotalNumberCustomer FROM Customers GROUP BY Country;
...
CREATE TABLE Orders (
    ...
    FOREIGN KEY (ProductID) REFERENCES Products(ProductID)); --CHUNMADIM
...
--TASK9;
SELECT MIN(Quantity) AS MinQuantity, MAX(Quantity) AS MaxQuantity FROM Orders; --or
...
--TASK10;
SELECT DISTINCT CustomerID FROM Orders WHERE OrderDate >= '2023-01-01' AND OrderDate <= '2023-01-31'
EXCEPT SELECT DISTINCT CustomerID FROM Invoices; --or
SELECT DISTINCT o.CustomerID FROM Orders o LEFT JOIN Invoices i ON o.CustomerID = i.CustomerID WHERE o.OrderDate >= '2023-01-01' AND o.OrderDate >= '2023-01-31'
...
SELECT ProductName, 'Regular' AS Source FROM Products UNION ALL SELECT ProductName, 'Discounted' AS Source FROM Products_Discounted;
--TASK12;
SELECT ProductName FROM Products UNION SELECT ProductName FROM Products_Discounted;
--TASK13;
SELECT YEAR(OrderDate) AS OrderYear, AVG(TotalAmount) AS AvgTotalAmount FROM Orders GROUP BY YEAR(OrderDate) ORDER BY OrderYear;
--TASK14;
...
--TASK15;
SELECT district_id, district_name, ISNULL([2012], 0) AS [2012], ISNULL([2013], 0) AS [2013]
INTO Population_Each_Year FROM (SELECT district_id, district_name, population, year FROM City_Population) AS SourceTable
PIVOT (SUM(population) FOR year IN ([2012], [2013])) AS PivotTable; --CHUNMADIM
...
--TASK18;
SELECT year, ISNULL([Bektemir], 0) AS Bektemir, ISNULL([Chilonzor], 0) AS Chilonzor, ISNULL([Yakkasaroy], 0) AS Yakkasaroy
INTO Population_Each_City FROM (SELECT year, district_name, population FROM city_population) AS SourceTable PIVOT
(SUM(population) FOR district_name IN ([Bektemir], [Chilonzor], [Yakkasaroy])) AS PivotTable;
SELECT*FROM Population_Each_City;
--TASK19;
SELECT TOP 3 CustomerID, SUM(TotalAmount) AS TotalSpent FROM Invoices GROUP BY CustomerID ORDER BY TotalSpent DESC;
--TASK20;
SELECT district_id, district_name, CAST(year AS VARCHAR(20)) AS year, population FROM (SELECT district_id, district_name, [2012], [2013]
FROM Population_Each_Year) AS PivotedData UNPIVOT (population FOR year IN ([2012], [2013])) AS UnpivotedData;
--TASK21;
SELECT p.ProductName, COUNT(s.SaleID) AS TimesSold FROM Products p JOIN Sales s ON p.ProductID = s.ProductID
GROUP BY p.ProductName ORDER BY TimesSold DESC;
--TASK22;
DROP TABLE City_Population;
SELECT Year, City AS district_name, Population INTO City_Population FROM Population_Each_City 
UNPIVOT (Population FOR City IN ([Bektemir], [Chilonzor], [Yakkasaroy])) AS unpvt;
SELECT*FROM City_Population;
