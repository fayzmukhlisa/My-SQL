...
--TASK4;
SELECT COUNT (DISTINCT Category) AS UniqueCateg FROM Products;
...
--TASK11;
SELECT p.Category, SUM(s.SaleAmount) AS TotalSales, AVG(s.SaleAmount) AS AvgSales FROM Sales s JOIN Products p ON s.ProductID = p.ProductID GROUP BY p.Category; 
--TASK12;
SELECT COUNT(*) AS HREmployees FROM Employees WHERE DepartmentName = 'HR';
SELECT COUNT(CASE WHEN DepartmentName = 'HR' THEN 1 END) AS HREmployees FROM Employees;
SELECT DepartmentName, COUNT (*) AS HREmployees FROM Employees WHERE DepartmentName = 'HR' GROUP BY DepartmentName;
--TASK13;
...
--TASK15;
SELECT DepartmentName, AVG(Salary) AS AvgSalary, COUNT(*) AS EmployeesCount FROM Employees GROUP BY DepartmentName;
--TASK16;
...
--TASK19;
SELECT DepartmentName, AVG(Salary) AS AvgSalary FROM Employees GROUP BY DepartmentName HAVING AVG(Salary) > 60000;
--TASK20;
...
--TASK22;
SELECT DepartmentName, SUM(Salary) AS TotalSalary, AVG(Salary) AS AvgSalary FROM Employees GROUP BY DepartmentName HAVING AVG(Salary) > 65000;
--TASK23;
SELECT custid, SUM(freight) AS TotalAmountOver50, MIN(freight) AS LeastPurchase FROM Sales.Orders WHERE freight > 50
GROUP BY custid ORDER BY custid;
--TASK24;
SELECT YEAR(OrderDate) AS OrderYear, MONTH(OrderDate) AS OrderMonth, SUM(TotalAmount) AS TotalSales, COUNT(DISTINCT ProductID) AS UniqueProducts 
FROM Orders GROUP BY YEAR(OrderDate), MONTH(OrderDate) HAVING COUNT(DISTINCT ProductID) >= 2 ORDER BY OrderYear, OrderMonth;
--TASK25;
SELECT YEAR(OrderDate) AS OrderYear, MIN(Quantity) AS MinOrder, MAX(Quantity) AS MaxOrder FROM Orders GROUP BY YEAR(OrderDate) ORDER BY OrderYear;
