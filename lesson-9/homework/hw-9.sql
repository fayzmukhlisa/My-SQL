...
SELECT*FROM Orders;
--TASK6;
SELECT p.ProductName, o.OrderID FROM Orders o
...
--TASK8;
SELECT s.Name AS StudentName, e.CourseID FROM Students s
...
SELECT*FROM Orders;
--TASK9;
SELECT o.OrderID, p.PaymentID, p.PaymentDate, p.Amount, p.PaymentMethod FROM Orders o 
INNER JOIN Payments p ON o.OrderID = p.OrderID; --CHUNMADIM

SELECT*FROM Orders;
SELECT*FROM Products;
--TASK10;
SELECT o.OrderID, p.ProductName, p.Price, o.Quantity, o.TotalAmount FROM Orders o
INNER JOIN Products p ON o.ProductID = p.ProductID WHERE p.Price > 100;

SELECT*FROM Employees;
SELECT*FROM Departments;
--TASK11;
SELECT e.Name AS EmployeeName, d.DepartmentName FROM Employees e
CROSS JOIN Departments d WHERE e.DepartmentID <> d.DepartmentID;

SELECT*FROM Orders;
SELECT*FROM Products;
--TASK12;
SELECT o.OrderID, p.ProductName, o.Quantity AS OrderedQuantity, p.StockQuantity AS AvailableStock 
FROM Orders o
INNER JOIN Products p ON o.ProductID = p.ProductID WHERE o.Quantity > p.StockQuantity;
CREATE TABLE Sales (
...
SELECT*FROM Sales;
--TASK13;
SELECT s.ProductID, c.FirstName, c.LastName, s.SaleAmount AS SaleAmounMoreThan500 
FROM Sales s INNER JOIN Customers c ON s.CustomerID = c.CustomerID WHERE s.SaleAmount >= 500; --or
SELECT 
...
INNER JOIN Suppliers s ON p.SupplierID = s.SupplierID WHERE s.SupplierName LIKE '%TECH%';

SELECT*FROM Orders;
SELECT*FROM Payments;
--TASK16;
SELECT o.CustomerID, o.OrderID, p.Amount AS LessThenTotal, o.TotalAmount FROM Orders o 
INNER JOIN Payments p ON o.OrderID = p.OrderID WHERE p.Amount < o.TotalAmount; 

SELECT*FROM Employees;
SELECT*FROM Departments;
--TASK17;
SELECT e.Name AS EmployeeCount, d.DepartmentName FROM Employees e
INNER JOIN Departments d ON d.DepartmentID = e.DepartmentID; --OR
SELECT e.Name AS EmployeeCount, d.DepartmentName FROM Employees e 
CROSS JOIN Departments d WHERE d.DepartmentID = e.DepartmentID;
...
INNER JOIN Categories c ON p.Category = c.CategoryID WHERE c.CategoryName IN ('Electronics', 'Furniture'); --or
SELECT 
    ProductName,
    Category
FROM Products
...
--TASK19; 
SELECT s.SaleID, s.ProductID, s.SaleDate, s.SaleAmount, c.FirstName + ' ' + c.LastName AS CustomerName, c.Country
FROM Sales s 
INNER JOIN Customers c ON s.CustomerID = c.CustomerID
WHERE c.Country = 'USA'; 
--TASK20;
SELECT o.OrderID, o.TotalAmount, o.OrderDate, c.FirstName + ' ' + c.LastName 
AS CustomerName, c.Country FROM Orders o
INNER JOIN Customers c ON o.CustomerID = c.CustomerID
WHERE c.Country = 'Germany' AND o.TotalAmount > 100;
--TASK21;
SELECT e1.Name AS Employee1, e1.DepartmentID AS Dept1, e2.Name AS Employee2, e2.DepartmentID AS Dept2
FROM Employees e1
