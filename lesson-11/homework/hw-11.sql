...
JOIN Customers c ON o.CustomerID = c.CustomerID WHERE OrderDate > '2022-12-31' ORDER BY OrderDate; 
...
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName IN('Sales', 'Marketing'); 
...
SELECT*FROM Orders;
--TASK4; 
SELECT c.FirstName +''+ c.LastName AS CustomerName, o.OrderID, o.OrderDate, c.Country FROM Customers c 
JOIN Orders o ON c.CustomerID = o.CustomerID WHERE YEAR(o.OrderDate) = 2023 AND c.Country = 'USA' ORDER BY o.OrderDate; 

SELECT*FROM Orders; 
SELECT*FROM Customers;
--TASK5; 
SELECT c.FirstName +''+ c.LastName AS CustomerName, COUNT(o.OrderID) AS TotalOrders FROM Customers c
LEFT JOIN Orders o ON o.CustomerID = c.CustomerID GROUP BY c.FirstName, c.LastName ORDER BY TotalOrders; 
...
JOIN Suppliers s ON p.SupplierID = s.SupplierID WHERE s.SupplierName IN ('Gadget Supplies','Clothing Mart'); 

SELECT*FROM Customers; 
SELECT*FROM Orders; 
--TASK7; 
SELECT c.FirstName +''+ c.LastName AS CustomerName, MAX(o.OrderDate) AS MostRecentOrderDate FROM Customers c
LEFT JOIN Orders o ON c.CustomerID = o.CustomerID GROUP BY c.FirstName, c.LastName ORDER BY MostRecentOrderDate DESC, CustomerName;

SELECT*FROM Orders; 
SELECT*FROM Customers; 
--TASK8; 
SELECT c.FirstName +''+ c.LastName AS CustomerName, o.TotalAmount AS OrderTotal FROM Orders o 
JOIN Customers c ON c.CustomerID = o.CustomerID WHERE o.TotalAmount > 500 ORDER BY TotalAmount DESC; 
...
JOIN Products p ON p.ProductID = s.ProductID WHERE YEAR(s.SaleDate) = 2022 OR s.SaleAmount > 400 ORDER BY s.SaleDate; 

SELECT*FROM Sales;
SELECT*FROM Products;
--TASK10;
SELECT p.ProductName, COALESCE(SUM(s.SaleAmount), 0) AS TotalSalesAmount FROM Products p
LEFT JOIN Sales s ON p.ProductID = s.ProductID GROUP BY p.ProductID, p.ProductName ORDER BY TotalSalesAmount DESC; 

SELECT*FROM Employees; 
SELECT*FROM Departments; 
--TASK11; 
SELECT e.Name AS EmployeeName, d.DepartmentName, e.Salary FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName = 'Human Resources' AND e.Salary > 60000; 
--TASK12;
SELECT p.ProductName, s.SaleDate, p.StockQuantity FROM Sales s
JOIN Products P ON s.ProductID = p.ProductID WHERE YEAR(s.SaleDate) = 2023 AND p.StockQuantity > 100 ORDER BY s.SaleDate;
--TASK13;
SELECT e.Name AS EmployeeName, d.DepartmentName, e.HireDate FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName = 'Sales' OR e.HireDate > '2020-12-31' ORDER BY e.HireDate; 

SELECT*FROM Customers; 
SELECT*FROM Orders; 
--TASK14; 
SELECT c.FirstName +''+ c.LastName AS CustomerName, o.OrderID, c.Address, o.OrderDate, c.Country FROM Customers c
JOIN Orders o ON c.CustomerID = o.CustomerID WHERE c.Country = 'USA' AND c.Address LIKE '[0-9][0-9][0-9][0-9]%'; 

SELECT*FROM Products;
SELECT*FROM Sales; 
--TASK15; 
SELECT p.ProductName, p.Category, s.SaleAmount FROM Products p
JOIN Sales s ON p.ProductID = s.ProductID WHERE p.Category = 'Electronics' OR s.SaleAmount > 350; 
...
LEFT JOIN Products p ON P.Category = C.CategoryID GROUP BY c.CategoryName;

UPDATE P 
SET P.Category = C.CategoryID
FROM Products P  JOIN Categories C ON P.Category=C.CategoryName; --SHUNI SORASHIM KERE

SELECT*FROM Customers; 
SELECT*FROM Orders; 
--TASK17; 
SELECT c.FirstName +''+ c.LastName AS CustomerName, c.City, o.OrderID, o.TotalAmount AS Amount FROM Customers c 
JOIN Orders o ON c.CustomerID = o.CustomerID WHERE c.City = 'Los Angeles' AND o.TotalAmount > 300; 

SELECT*FROM Employees; 
SELECT*FROM Departments; 
--TASK18; 
SELECT e.Name AS EmployeeName, d.DepartmentName FROM Employees e 
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName IN('Human Resources', 'Finance') OR (e.Name) LIKE '%a%e%i%o%'; --CHUNMADIM SORASHIM SHART!!
--TASK19; 
SELECT e.Name AS EmployeeName, d.DepartmentName, e.Salary FROM Employees e 
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName IN('Sales', 'Marketing') OR e.Salary > 60000;
