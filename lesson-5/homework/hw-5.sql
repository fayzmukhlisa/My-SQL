...
--TASK6
SELECT Price, CASE WHEN Price > 1000 THEN 'High' WHEN Price <= 1000 THEN 'Low' END AS PriceCategory FROM Products;
--TASK7
SELECT StockQuantity, IIF(StockQuantity > 100, 'Yes', 'No') AS StockLevel FROM Products_Discounted;
--TASK8
SELECT ProductName FROM Products UNION SELECT ProductName FROM Products_Discounted;
--TASK9
SELECT * FROM Products EXCEPT SELECT * FROM Products_Discounted;
--TASK10
SELECT Price, IIF(Price > 1000, 'Expensive', 'Affordable') AS PriceCategory FROM Products;
...
SELECT * FROM Sales;
--TASK13
SELECT SaleID, ProductID, CustomerID, SaleAmount, CASE WHEN SaleAmount > 500 THEN 'TopTier' WHEN SaleAmount BETWEEN 200 AND 500 THEN 'MidTier' ELSE 'LowTier' 
END AS SalesCategory FROM Sales;
...
--TASK15
SELECT CustomerID, Quantity, CASE WHEN Quantity = 1 THEN '3%' WHEN Quantity BETWEEN 1 AND 3 THEN '5%' ELSE '7%' END AS DiscountPercentage FROM Orders;
