...
INNER JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE e.Salary > 50000;
...
SELECT*FROM Orders;
--TASK2;
SELECT c.FirstName +''+ c.LastName AS CustomerName, o.OrderDate FROM Customers c 
INNER JOIN Orders o ON c.CustomerID = o.CustomerID WHERE YEAR(o.OrderDate) = 2023; --OR
SELECT 
    c.FirstName,
    c.LastName,
    o.OrderDate
...
LEFT JOIN Departments d ON e.DepartmentID = d.DepartmentID; --ISNULL BILAN QILSA BOMIDIMI? HULLAS, O'ZI ISNULL NEGA KERAK EDI?
...
SELECT*FROM Employees; 
--TASK6; 
SELECT e.Name AS EmployeeName, m.ManagerID AS ManagerName FROM Employees e
LEFT JOIN Employees m ON e.ManagerID = m.EmployeeID; -- self joinda obyazatelno joinlani bittasi qatnashadi? toest self join degan code yoq?
...
--TASK9;
SELECT e.Name AS EmployeeName, d.DepartmentName FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName = 'Human Resources'; 
--TASK10;
SELECT d.DepartmentName, COUNT(e.EmployeeID) AS EmployeeCount FROM Employees e 
JOIN Departments d ON e.DepartmentID = d.DepartmentID GROUP BY DepartmentName HAVING COUNT(e.EmployeeID) > 5;
...
SELECT*FROM Orders;
--TASK12; 
SELECT DISTINCT c.FirstName, c.LastName, o.Quantity AS TotalOrders FROM Customers c
...
JOIN Employees e2 ON e1.ManagerID = e2.ManagerID AND e1.EmployeeID < e2.EmployeeID; --OR SHU JOYIDAYAM CHUNMI QOLDIM 
SELECT e1.Name AS Employee1, e2.Name AS Employee2, e1.ManagerID FROM Employees e1 
JOIN Employees e2 ON e1.ManagerID = e2.ManagerID AND e1.EmployeeID < e2.EmployeeID WHERE e1.ManagerID IS NOT NULL; 

SELECT*FROM Orders;
SELECT*FROM Customers; 
--TASK15; 
SELECT o.OrderID, o.OrderDate, c.FirstName, c.LastName FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID WHERE YEAR(o.OrderDate) = 2022; 
...
--TASK18; 
SELECT p.ProductID, p.ProductName FROM Products p
LEFT JOIN Orders o ON p.ProductID = o.ProductID WHERE o.OrderID IS NULL;

SELECT*FROM Employees;
--TASK19; 
SELECT e.Name AS EmployeeName, e.Salary FROM Employees e WHERE e.Salary > (SELECT AVG(e2.Salary) 
FROM Employees e2 WHERE e2.DepartmentID = e.DepartmentID); 
SELECT 
...
WHERE e.Salary > dept_avg.AvgSalary; --QAYTARISHIM SHART!! 

SELECT*FROM Orders;
SELECT*FROM Payments;
--TASK20;
SELECT o.OrderID, o.OrderDate FROM Orders o
LEFT JOIN Payments p ON o.OrderID = p.OrderID WHERE o.OrderDate < '2020-01-01' AND p.PaymentID IS NULL; 
...
--TASK21; 
SELECT p.ProductID, p.ProductName FROM Products p
LEFT JOIN Categories c ON p.Category = c.CategoryID WHERE c.CategoryID IS NULL;
...
JOIN Employees e2 ON e1.ManagerID = e2.ManagerID WHERE e1.Salary > 60000 AND e1.EmployeeID < e2.EmployeeID; --SHU IKKI QUERY FARQINI CHUNTIRISH KERE
...
JOIN Employees e2
   ...
   AND e1.EmployeeID < e2.EmployeeID  -- avoid duplicates & self-pairing
...
--TASK23; 
SELECT e.Name AS EmployeeName, d.DepartmentName FROM Employees e
JOIN Departments d ON e.DepartmentID = d.DepartmentID WHERE d.DepartmentName LIKE 'M%'; 

SELECT*FROM Products; 
SELECT*FROM Sales; 
--TASK24; 
SELECT s.SaleID, p.ProductName, s.SaleAmount FROM Products p
JOIN Sales s ON p.ProductID = s.ProductID WHERE s.SaleAmount > 500; 
...
JOIN Courses c ON c.CourseID = e.CourseID WHERE c.CourseName = 'Math 101'); -- BUNI QAYTARIB, YAXXXSHILAB CHUNVOLISHIM KERE!!!! 

SELECT*FROM Orders;
SELECT*FROM Payments;
--TASK26; 
SELECT o.OrderID, o.OrderDate, p.PaymentID FROM Orders o
LEFT JOIN Payments p ON o.OrderID = p.OrderID WHERE p.PaymentID IS NULL; 

SELECT*FROM Products;
SELECT*FROM Categories; 
--TASK27; 
SELECT p.ProductID, p.ProductName, c.CategoryName FROM Products p 
JOIN Categories c ON p.Category = c.CategoryID WHERE c.CategoryName IN ('Electronics', 'Furniture');
